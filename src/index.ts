...
export default class ClusterWS {
  ...
  private masterProcess(options: Options): void {
    ...
    if (options.horizontalScaleOptions && options.horizontalScaleOptions.masterOptions) launchProcess('Scaler', -1);
    ...
    function ready(processName: string, processId: number, pid: number): void {
      if (isReady) return logReady(`${processName} PID ${pid} has been restarted`);

      if (processName === 'Worker') workersReady[processId] = `\tWorker: ${processId}, PID ${pid}`;

      if (processName === 'Scaler') for (let i: number = 0; i < options.brokers; i++) launchProcess('Broker', i);

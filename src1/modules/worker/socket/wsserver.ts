...
export class WSServer extends EventEmitterSingle {
  ...
  public publish(channel: string, message: Message, tryiesOnBrokerError: number = 0): void | NodeJS.Timer {
    if (tryiesOnBrokerError > this.internalBrokers.brokersAmount + 10) return logWarning('Does not have access to any broker')
    if (this.internalBrokers.brokersAmount <= 0)
      return setTimeout(() => this.publish(channel, message, ++tryiesOnBrokerError), 10)
    ...
    this.internalBrokers.brokers[this.internalBrokers.brokersKeys[this.internalBrokers.nextBroker]].send(
      Buffer.from(channel + '%' + JSON.stringify({ message }))
    )
  ...
  public broadcastMessage(x: string, message: Message): void {
    ...
    if (channel === '#sendToWorkers')
      return (
        this.middleware.onMessageFromWorker &&
